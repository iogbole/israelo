---
layout: post
title:  "Slash Cloud Costs by 14%: A Beginner's Guide to Profile Guided Optimization in Go"
author: israel
...
Over the past few months, I've had numerous discussions with customers and colleagues about the benefits of Profile-Guided Optimization (PGO). It’s a topic that sparks interest, but many find it challenging to get started or lack the time to explore it fully. As a Product Manager in the continuous profiling domain, my curiosity led me to dive deeper into this subject. After studying several academic papers and articles on the subject, I decided to roll up my sleeves and implement PGO myself, benchmarking its impact to determine if it’s truly worthwhile. My goal was to understand what might be holding customers back from using it and to identify key questions that could uncover their real pain points. This blog summarizes my initial findings.

-   This blog presents a practical implementation of PGO, along with tips on measuring performance gains through inlining, binary size, and flamegraphs. 

-   In the example code below, my analysis shows a performance gain of ~ `6.92%` in compute efficiency. I wasn't expecting such improvement considering it's only a small JSON unmarshalling code. The potential savings in your production could be more. 

-   PGO can significantly enhance code performance and resource utilization, potentially saving up to `14%` on cloud spending without requiring any code changes.
    
-   Many developers and SREs are missing out on potential cost savings by not leveraging PGO. Learn from Cloudflare's [experience](https://blog.cloudflare.com/reclaiming-cpu-for-free-with-pgo).
- You need continous profiling in production to fully realize the benefits of PGO. 
...
To connect the dots, the way a coach observes an athlete's training regimen and physiological components is similar to `profiling` in this context. Just as a coach **scouts out** their athletes, **continuous profiling** analyzes your code's behavior on an ongoing basis.
...
Futher, PGOs can improve your code's resouce utilization efficiency by upto `14%`. That's alot of cost savings in most medium - to large organisations without making any code changes, that's free money on the table!
...
*Comparing the before and after flamegraps of PGO. See details in the result section below* 
...
|---------------|-----------------|------------------------------|----------------------------------------------------------------------------------|
| **`pre_pgo`** | 8,162,050 bytes | N/A                          | This is the size of the binary before applying Profile-Guided Optimization (PGO). |
...
8.  **Fortran**: Supported in GFortran as part of GCC with the same options as C/C++.  *Does anyone strill write Fortran code?*  
...
## References 
-   https://go.dev/doc/pgo#overview
-   https://research.nvidia.com/publication/2021-07_cooperative-profile-guided-optimization
-   https://github.com/google/autofdo 
-   https://www.researchgate.net/publication/222669095_PGO_a_Parallel_Computing_Platform_for_Global_Optimization_Based_on_Genetic_Algorithm